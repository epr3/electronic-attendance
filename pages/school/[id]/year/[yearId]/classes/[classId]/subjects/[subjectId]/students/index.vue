...
const { data, refresh } = await useAsyncData(
  `students-${route.params.subjectId}`,
  async () => {
    const students = await $client.user.getStudentsBySubject.query({
...
const removeStudentFromSubject = async (studentId: string) => {
  await $client.schedule.removeStudentFromSchedule.mutate({
    studentId,
...
<template>
  <div class="flex flex-col gap-4">
    <Button
      color="success"
      class="self-start"
      :to="`/school/${route.params.id}/year/${route.params.yearId}/classes/${route.params.classId}/subjects/${route.params.subjectId}/students/new`"
    >
      Add student
    </Button>
    <Table full-width>
          ...
          <TableRow v-for="row in students" :key="row.id">
            ...
            <TableCell>
              <div class="flex space-x-4">
                ...
                <IconButton
                  color="error"
                  @click="removeStudentFromSubject(row.id)"
                >
                  <div class="i-heroicons-user-minus w-6 h-6" />
                </IconButton>
              </div>
            </TableCell>
          </TableRow>
